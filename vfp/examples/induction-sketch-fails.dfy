...
lemma theorem_preservation(t: tm)
  requires has_type(map[], t).Some?
  requires {:induction_target} step(t).Some?
  ensures has_type(map[], step(t).get) == has_type(map[], t)
{
    ...
    if (t.tapp? && t.f.tabs? && value(t.arg)) {
      assert has_type(map[], t.f).Some?;
      assert has_type(map[], t.arg).Some?;
      assert has_type(map[], t.f).get.TArrow?;
      assert has_type(map[], t.f).get.T1 == has_type(map[], t.arg).get;
...
lemma corollary_soundness({:induction_target} t: tm, t': tm, T: ty, n: nat)
...
{
  // DESCRIPTION: 
  // generated induction sketch does not resolve
  // resolution fails on line 572

      // Inductive proof using rule induction
    // following function: has_type
    match t {
        case tvar(id) => {
        }
        case tabs(x, T, body) => {
            var ty_body := has_type(extend(x, T, map[]), body);
            corollary_soundness(body, t', T, n);
            if (ty_body.Some?) {
            } else {
            }
        }
        case tapp(f, arg) => {
            ...
            corollary_soundness(f, t', T, n);
            var ty_arg := has_type(map[], arg);
            corollary_soundness(arg, t', T, n);
            if (ty_f.Some? && ty_arg.Some?) {
                if (ty_f.get.TArrow? && ty_f.get.T1 == ty_arg.get) {
                } else {
                }
            } else {
            }
        ...
        case tif(cond, a, b) => {
            ...
            corollary_soundness(cond, t', T, n);
            var ty_a := has_type(map[], a);
            corollary_soundness(a, t', T, n);
            var ty_b := has_type(map[], b);
            corollary_soundness(b, t', T, n);
            if (ty_c.Some? && ty_a.Some? && ty_b.Some?) {
                if (ty_c.get == ty.TBool && ty_a.get == ty_b.get) {
                } else {
                }
            } else {
            }
        ...
        case tprev(n) => {
            var ty_n := has_type(map[], n);
            corollary_soundness(n, t', T, n);
            if (ty_n.Some?) {
                if (ty_n.get == ty.TNat) {
                } else {
                }
        ...
        case tsucc(p) => {
            var ty_p := has_type(map[], p);
            corollary_soundness(p, t', T, n);
            if (ty_p.Some?) {
                if (ty_p.get == ty.TNat) {
                } else {
                }
        ...
        case teq(n1, n2) => {
            var ty_n1 := has_type(map[], n1);
            corollary_soundness(n1, t', T, n);
            var ty_n2 := has_type(map[], n2);
            corollary_soundness(n2, t', T, n);
            if (ty_n1.Some? && ty_n2.Some?) {
                if (ty_n1.get == ty.TNat && ty_n2.get == ty.TNat) {
                } else {
                }
            } else {
            }
        ...
        case tfold(U, t1) => {
            var ty_t1 := if ty_closed(U) then has_type(map[], t1) else option<ty>.None;
            if (ty_closed(U)) {
                corollary_soundness(t1, t', T, n);
            } else {
            }
            if (ty_t1.Some?) {
                if (U.TRec? && ty_t1.get == tsubst(U.X, U, U.T)) {
                } else {
                }
            } else {
            }
